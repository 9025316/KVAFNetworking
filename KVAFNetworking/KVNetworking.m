...
#define KV_ERROR_IMFORMATION @"网络出现错误，请检查网络连接"

#define KV_ERROR [NSError errorWithDomain:@"com.hyq.KVNetworking.ErrorDomain" code:-999 userInfo:@{ NSLocalizedDescriptionKey:KV_ERROR_IMFORMATION}]

static NSMutableArray   *requestTasksPool;

static NSDictionary     *headers;

static KVNetworkStatus  networkStatus;

static NSTimeInterval   requestTimeout = 20.f;

@implementation KVNetworking
#pragma mark - manager
+ (AFHTTPSessionManager *)manager {
    ...
    [[KVCacheManager shareManager] clearLRUCache];
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusNotReachable:
                networkStatus = KVNetworkStatusNotReachable;
                break;
            case AFNetworkReachabilityStatusUnknown:
                networkStatus = KVNetworkStatusUnknown;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
                networkStatus = KVNetworkStatusReachableViaWWAN;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
                networkStatus = KVNetworkStatusReachableViaWiFi;
                break;
            default:
                networkStatus = KVNetworkStatusUnknown;
                break;
...
+ (KVURLSessionTask *)getWithUrl:(NSString *)url
    ...
    __block KVURLSessionTask *session = nil;
    
    AFHTTPSessionManager *manager = [self manager];
    
    if (networkStatus == KVNetworkStatusNotReachable) {
        if (failBlock) failBlock(KV_ERROR);
        return session;
    }
    
    id responseObj = [[KVCacheManager shareManager] getCacheResponseObjectWithRequestUrl:url params:params];
    ...
    session = [manager GET:url
                parameters:params
                  progress:^(NSProgress * _Nonnull downloadProgress) {
                      ...
                      if (cache) [[KVCacheManager shareManager] cacheResponseObject:responseObject requestUrl:url params:params];
    ...
    if ([self haveSameRequestInTasksPool:session] && !refresh) {
        ...
        KVURLSessionTask *oldTask = [self cancleSameRequestInTasksPool:session];
...
+ (KVURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    NSURL *fileUrl = [[KVCacheManager shareManager] getDownloadDataFromCacheWithRequestUrl:url];
                ...
                parameters:nil
                  progress:^(NSProgress * _Nonnull downloadProgress) {
                      ...
                      if (successBlock) {
                          ...
                          [[KVCacheManager shareManager] storeDownloadData:dataObj requestUrl:url];
                          
                          NSURL *downFileUrl = [[KVCacheManager shareManager] getDownloadDataFromCacheWithRequestUrl:url];
...
+ (void)cancleAllRequest {
    @synchronized (self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(KVURLSessionTask  *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[KVURLSessionTask class]]) {
                [obj cancel];
            }
        }];
...
@implementation KVNetworking (cache)
+ (NSUInteger)totalCacheSize {
    return [[KVCacheManager shareManager] totalCacheSize];
}

+ (NSUInteger)totalDownloadDataSize {
    return [[KVCacheManager shareManager] totalDownloadDataSize];
}

+ (void)clearDownloadData {
    [[KVCacheManager shareManager] clearDownloadData];
}

+ (NSString *)getDownDirectoryPath {
    return [[KVCacheManager shareManager] getDownDirectoryPath];
}

+ (NSString *)getCacheDiretoryPath {
    
    return [[KVCacheManager shareManager] getCacheDiretoryPath];
}

+ (void)clearTotalCache {
    [[KVCacheManager shareManager] clearTotalCache];
}
